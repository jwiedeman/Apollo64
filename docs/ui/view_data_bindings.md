...
| Audio Status | `frame.audio` (binder/dispatcher summaries) | `audioBinder` / `audioDispatcher` (for expanded queues) | Shows last cue ID, active/queued bus counts, and pending binder triggers so captions and mute toggles stay in sync. | Clicking opens audio console overlay or cycles caption verbosity; long press mutes active alerts. |
...
| Console Dock (Nav filter) | `missionLog` filtered to Navigation category | Adds inline chips linking to relevant panels/events; see [`console_dock.md`](console_dock.md) for global behaviour. | Filter chips toggle categories (Events/Autopilot/Audio). Search field jumps to matching log entries and highlights corresponding timeline markers. |
...
| DSKY Module | `frame.agc`, `frame.checklists.chip`, `docs/ui/dsky_macros.json`, `frame.autopilot.primary`, `missionLog` (`agc` category) | Macro tray shows applicable Verb/Noun combos with prerequisites resolved (`panels.json` dependencies). Display registers, annunciators, macro labels, and history mirror `frame.agc`. | Pressing macro button queues manual action; invalid state triggers warning toast + audio cue from taxonomy plan. Numeric keypad inputs feed manual queue `dsky_entry` actions that execute through `AgcRuntime`. |
