...
| State Vector Panel | `frame.resources.deltaV`, `frame.autopilot.primary`, PAD metadata (`pads.csv` entry referenced by active event) | Residuals compare current vs. planned; pull base/adjustment/usable Δv per stage from `deltaV.stages`; highlight if outside tolerance in `autopilot.primary`. | Copy button writes PAD to clipboard/Controller Pak log. `Plan Burn` opens DSKY macros `V21N33` + `V21N38` pre-filled with PAD values. |
