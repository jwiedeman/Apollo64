...
- Performance telemetry plan in [`docs/ui/performance_telemetry.md`](ui/performance_telemetry.md) keeps frame-time, audio-queue, input-latency, and N64 profiling metrics aligned with the mission log, UI frame exports, and parity tooling so HUD/audio regressions surface immediately.
- Mission replay workflow and annotation plan in [`docs/ui/replay_viewer.md`](ui/replay_viewer.md) keep exported `ui_frame` bundles, mission logs, manual action scripts, and audio cue ledgers reviewable without diverging from the Navigation/Controls/Systems cadence.
- Commander scorecard layout, thresholds, and Systems-view interactions are captured in [`docs/ui/commander_scorecard.md`](ui/commander_scorecard.md) so scoring telemetry from `ScoreSystem` renders consistently across browser and N64 builds.
...
- **Docking rendezvous control:** The blueprint in [`docs/sim/docking_simulation.md`](sim/docking_simulation.md) describes the deterministic docking state machine, braking-gate evaluation, manual/autopilot parity hooks, scoring effects, and parity harness instrumentation that will guide Milestone M2's rendezvous implementation.
